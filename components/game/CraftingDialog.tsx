...
export function CraftingDialog() {
  ...
  // 关闭时直接不渲染内容：背包槽位高频变化不触发配方重算与 JSX（hooks 已全部调用，顺序稳定）
  if (!open) return null;
  return (
...
/** 背包/工作台面板内容（仅打开时挂载）：配方书 + 摆法预览 + 槽位区 */
function CraftingContent() {
  ...
  // 合并背包：仅槽位数组引用变化才新建（hover/cursorSlot/dragenter 变化不触发）
  const merged = useMemo(() => [...hotbarSlots, ...mainSlots], [hotbarSlots, mainSlots]);
  const recipes = useMemo(() => RECIPES.filter((r) => withTable || !r.needsTable), [withTable]);
  // 全配方可合成集合：~100 配方 × 45 格扫描只在背包引用变化时重算一次，渲染期 O(1) 查表
  const craftable = useMemo(() => {
    const ok = new Set<string>();
    for (const r of recipes) if (canCraft(merged, r)) ok.add(r.id);
    return ok;
  }, [merged, recipes]);
  // 预览 = 悬停配方，默认第一个可合成的（让摆法区不空）
  const preview = hover ?? recipes.find((r) => craftable.has(r.id)) ?? recipes[0] ?? null;
  ...
  const outOk = preview ? craftable.has(preview.id) : false;
